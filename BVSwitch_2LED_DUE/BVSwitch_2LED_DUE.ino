...
volatile int LED_array[] = {2, 3}; // Order is blue lime green red {14,15}; //{DAC0,DAC1}; {52,51};//{8,10,12,6}; //these pins have LEDs attached to them.  (DAC0 and DAC1 are true analog output pins / referred to as DAC1 and DAC2 on the board)
...
{ 
  while (SerialUSB.available() > 0) //(SerialUSB.available()>0)     // run only when receiving data
  {
    incoming_value = SerialUSB.read(); //SerialUSB.read();      // read value from native port (IMPORTANT!!!!!!!!!!)
    ...
    case 99:  // c    (clear: disable interrupts for delay pin -> turn off all LEDs -> free dynamic memory)
      ...
      SerialUSB.print("Interrupts disabled");  //SerialUSB.print("Interrupts disabled"); 
    ...
    case 116:  // t   (test: turn off all LEDs -> 10ms delay -> grab toggle index -> toggle on indexed LED)
      ...
      ledToggleIndex = SerialUSB.read() - 49; //SerialUSB.read() - 49;   // ASCII equivalent for .read - 1;
      ...
      SerialUSB.print(ledToggleIndex+1,DEC); //"SerialUSB.print(ledToggleIndex+1,DEC);
      
      break;


    case 120:  // x   (All Off)
      SerialUSB.println("Toggle all LEDs OFF"); //SerialUSB.println("Toggle all LEDs OFF");
      turn_off_all_LEDs();
      break;
    ...
    case 115:  // s   (Strobe: read available bytes -> allocate memory of LED order array and initiate values -> read order -> strobe LEDs)
      ...
      numberOfElements = SerialUSB.available(); //SerialUSB.available();
      ...
      for (int counter = 0; counter < numberOfElements; counter++)   {
        LED_order[counter] = SerialUSB.read() - 49; //SerialUSB.read() - 49;   // ACSII equivalent for .read - 1
      }

      // Print LED_order to serial port
      SerialUSB.println("LED_order is:"); //SerialUSB.println("LED_order is:");
      for (int counter = 0; counter < numberOfElements; counter++)   {
         SerialUSB.println(LED_order[counter], DEC); //SerialUSB.println(LED_order[counter], DEC);
      }
      // CJ: FROM HERE I JUST REPLACED ALL serialUSB by Serial
