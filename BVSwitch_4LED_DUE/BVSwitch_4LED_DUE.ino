...
volatile int LED_array[] = {2, 3, 4, 5}; // Order is blue lime green red {14,15}; //{DAC0,DAC1}; {52,51};//{8,10,12,6}; //these pins have LEDs attached to them.  (DAC0 and DAC1 are true analog output pins / referred to as DAC1 and DAC2 on the board)
//volatile int Exposure_Output_pin = 4;   //this pin outputs the exposure signal read from the camera. // not actually used, the expose out signal is sent for the Behavioral nidq in he matlab routine for loop 
volatile int Stim_Delay_pin = 38;       // strobe signal (CMOS expose out), on arduino UNO only pins 2 and 3 can be used as interrupt so exposure out needs to bre on those pins
volatile int Stim_Delay_pin_off = 39;   // on the old set-up, this is just dooubling exposure out, not sure why we need two different pins for that
...
{
  Serial.begin(9600); //for DUE
  //Serial.begin(9600); //for UNO?
  for (int i=0; i<LENGTH_LED_ARRAY; i++) // *(int i=0; i<1; i++)*     // Set all LED pins to output with off states
  {
...
{ 
  while (Serial.available() > 0){ //(Serial.available()>0)     // run only when receiving data
    incoming_value = Serial.read(); //Serial.read();      // read value from native port (IMPORTANT!!!!!!!!!!)
    ...
    case 99:  // c    (clear: disable interrupts for delay pin -> turn off all LEDs -> free dynamic memory)
      ...
      Serial.print("Interrupts disabled");  //Serial.print("Interrupts disabled"); 
      break;
    ...
    case 116:  // t   (test: turn off all LEDs -> 10ms delay -> grab toggle index -> toggle on indexed LED)
      ...
      ledToggleIndex = Serial.read() - 49; //Serial.read() - 49;   // ASCII equivalent for .read - 1;
      //Serial.println(LED_array[ledToggleIndex], DEC);
      digitalWrite(LED_array[ledToggleIndex], LED_ON); // Note that UNO doesn't have a DAC (digital-to-analog conversion channels, just use digital channels)
      Serial.print(ledToggleIndex+1,DEC); //"Serial.print(ledToggleIndex+1,DEC);
      
      break;


    case 120:  // x   (All Off)
      Serial.println("Toggle all LEDs OFF"); //Serial.println("Toggle all LEDs OFF");
      turn_off_all_LEDs();
      break;
    ...
    case 115:  // s   (Strobe: read available bytes -> allocate memory of LED order array and initiate values -> read order -> strobe LEDs)
      ...
      numberOfElements = Serial.available(); //Serial.available();
      ...
      for (int counter = 0; counter < numberOfElements; counter++)   {
        LED_order[counter] = Serial.read() - 49; //Serial.read() - 49;   // ACSII equivalent for .read - 1
      }

      // Print LED_order to serial port
      Serial.println("LED_order is:"); //Serial.println("LED_order is:");
      for (int counter = 0; counter < numberOfElements; counter++)   {
         Serial.println(LED_order[counter], DEC); //Serial.println(LED_order[counter], DEC);
      }
      // CJ: FROM HERE I JUST REPLACED ALL Serial by Serial
...
void strobe_on_LEDs(){   // start delay (if needed) and allow interrupts -> turn on next LED in turn
  if (start_delay==1){
    interrupts();
    delayMicroseconds(10000); //5000 originally
    start_delay=0;
  }
...
void strobe_off_LEDs(){    // turn off LED in turn -> select next LED in turn
  ...
  // Re-enable start delay for the next cycle
  start_delay = 1;
}
